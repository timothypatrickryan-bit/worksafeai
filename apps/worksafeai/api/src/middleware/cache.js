...
const cachedResponse = (keyBuilder, ttl) => {
  return async (req, res, next) => {
    ...
    // Intercept res.json to cache the response — only cache successful responses.
    // Caching error responses (4xx/5xx) would poison the cache and serve stale
    // errors to subsequent requests even after the underlying issue is resolved.
