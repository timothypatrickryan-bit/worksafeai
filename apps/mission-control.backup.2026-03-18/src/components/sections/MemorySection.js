...
export default function MemorySection({ state }) {
  ...
  const loadMemories = async () => {
    try {
      ...
      const dailyRes = await fetch('http://localhost:3000/api/memories/load-daily')
      if (dailyRes.ok) {
        const dailyMemories = await dailyRes.json()
        setMemories(dailyMemories)
        if (dailyMemories.length > 0) {
          setSelectedMemory(dailyMemories[0])
        }
      }

      // Load long-term memory from MEMORY.md
      const longtermRes = await fetch('http://localhost:3000/api/memories/load-longterm')
      if (longtermRes.ok) {
        const longtermData = await longtermRes.json()
        if (longtermData.exists && longtermData.content) {
