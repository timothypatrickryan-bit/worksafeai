...
export default function TeamSection({ state }) {
  ...
  const handleGenerateAgent = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3000/api/team/generate-subagent', {
        method: 'POST',
  ...
  const handleCreateFromGenerated = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/team/subagent', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleEditSave = async () => {
    if (!editingId) return
    try {
      const response = await fetch(`http://localhost:3000/api/team/member/${editingId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
