...
export default function TaskProgressDashboard() {
  ...
  const loadMissionControlState = () => {
    try {
      // Parse the actual mission control state
      setTasks(missionControlState.tasks || []);
      setDelegations(missionControlState.delegations || []);
      setAgents(missionControlState.team?.members.reduce((acc, member) => {
        acc[member.id] = member;
        return acc;
      }, {}) || {});
    } catch (err) {
      console.error('Error loading mission control state:', err);
    } finally {
      setLoading(false);
    }
  };
