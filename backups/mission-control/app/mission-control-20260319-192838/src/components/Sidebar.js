...
const navItems = [
  // UNIFIED HUB — Single source of truth for all work
  { id: 'unified-dashboard', icon: '🎯', label: 'Unified Dashboard', countKey: 'unified' },
...
export default function Sidebar({ currentSection, setCurrentSection, state }) {
  const getCount = (key) => {
    if (!state) return 0
    if (key === 'briefings') {
      const pending = (state.tasks || []).filter(t => t.status === 'briefing').length
      return pending
    }
    if (key === 'tasks') {
