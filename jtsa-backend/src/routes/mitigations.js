...
router.patch('/mitigations/:id',
  ...
  async (req, res) => {
    try {
      ...
      // Check mitigation access
      const { data: mitigation } = await supabase
        .from('mitigations')
        .select('hazard:hazards(jtsa:jtsas(project:projects(company_id)))')
        .eq('id', req.params.id)
        .single();

      if (!mitigation || mitigation.hazard.jtsa.project.company_id !== req.user.companyId) {
        return res.status(403).json({ error: 'Access denied' });
      }
  ...
  async (req, res) => {
    try {
      ...
      const { data: mitigation, error } = await supabase
        ...
        .select(`
          *,
          hazard:hazards(*, jtsa:jtsas(project:projects(company_id)))
        `)
        .eq('id', req.params.id)
        .single();
